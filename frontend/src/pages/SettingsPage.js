...
function ImportTab() {
    ...
    const { data: parseStatus } = useQuery({
        ...
        enabled: !!docId,
        refetchInterval: (data) => data?.parseStatus === 'PARSING' || data?.parseStatus === 'PENDING' ? 2000 : false,
    });
    ...
    return (_jsxs(Card, { title: "Import Trade Confirmation (PDF)", subtitle: "Upload a broker contract note \u2014 we'll extract the trade details automatically.", children: [_jsxs("div", { onDragOver: e => { e.preventDefault(); setDragOver(true); }, onDragLeave: () => setDragOver(false), onDrop: e => { e.preventDefault(); setDragOver(false); const f = e.dataTransfer.files[0]; if (f)
                    ...
                    : 'border-gray-300 hover:border-gray-400 hover:bg-gray-50/60'), children: [_jsx(Upload, { size: 24, className: "mx-auto text-gray-400 mb-2" }), _jsx("p", { className: "text-sm font-medium text-gray-600", children: "Drop a PDF here or click to browse" }), _jsx("p", { className: "text-xs text-gray-400 mt-1", children: "CommSec and SelfWealth contract notes supported" }), _jsx("input", { ref: fileRef, type: "file", accept: ".pdf", className: "hidden", onChange: e => { const f = e.target.files?.[0]; if (f)
                            handleFile(f); } })] }), uploadMutation.isPending && (_jsxs("div", { className: "flex items-center gap-2 mt-3 text-sm text-gray-500", children: [_jsx(Loader2, { size: 14, className: "animate-spin" }), " Uploading\u2026"] })), parseStatus && (_jsx("div", { className: clsx('mt-3 p-4 rounded-xl border text-sm', parseStatus.parseStatus === 'PARSED'
                    ? 'bg-emerald-50 border-emerald-200 text-emerald-800'
                    : parseStatus.parseStatus === 'FAILED'
                        ? 'bg-red-50 border-red-200 text-red-800'
                        : 'bg-blue-50 border-blue-200 text-blue-800'), children: _jsxs("div", { className: "flex items-start gap-2", children: [statusIcon(), _jsxs("div", { className: "flex-1", children: [_jsxs("p", { className: "font-medium", children: [parseStatus.parseStatus === 'PARSED' && 'Trade extracted successfully', parseStatus.parseStatus === 'PARSING' && 'Parsing in progress…', parseStatus.parseStatus === 'PENDING' && 'Queued for parsing…', parseStatus.parseStatus === 'FAILED' && 'Parse failed'] }), parseStatus.brokerDetected && (_jsxs("p", { className: "text-xs mt-0.5", children: ["Detected broker: ", _jsx("strong", { children: parseStatus.brokerDetected }), parseStatus.confidence != null && (_jsxs("span", { className: "ml-2", children: ["(", Math.round(parseStatus.confidence * 100), "% confidence)"] }))] })), parseStatus.parseStatus === 'FAILED' && parseStatus.errorMessage && (_jsx("p", { className: "text-xs mt-1", children: parseStatus.errorMessage })), parseStatus.parseStatus === 'PARSED' && (_jsx("p", { className: "text-xs mt-1", children: "Review and confirm the extracted trade in the Trades page before it's saved." }))] })] }) })), _jsx("div", { className: "mt-4 text-xs text-gray-400 space-y-1", children: _jsx("p", { children: "The extracted trade data is shown for your review before being saved to your portfolio." }) })] }));
}
// =============================================================================
// Notifications tab
// =============================================================================
