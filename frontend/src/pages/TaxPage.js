...
export default function TaxPage() {
    ...
    const [parcelSortDir, setParcelSortDir] = useState('desc');
    ...
    const eventsQ = useQuery({
        ...
        queryFn: async () => {
            const r = await api.get(`/v1/portfolios/${portfolioId}/tax/cgt-events`, { params: { financialYear: fy, sort: eventSort, sortDir: eventSortDir } });
            return r.data;
        },
    ...
    const parcelsQ = useQuery({
        ...
        queryFn: async () => {
            const r = await api.get(`/v1/portfolios/${portfolioId}/tax/open-parcels`, { params: { sort: parcelSort, sortDir: parcelSortDir } });
            return r.data;
        },
    ...
    const dividendsQ = useQuery({
        ...
        queryFn: async () => {
            const r = await api.get(`/v1/portfolios/${portfolioId}/tax/dividends`, { params: { financialYear: fy } });
            return r.data;
        },
    ...
    return (_jsxs("div", { className: "p-8 space-y-6", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { children: [_jsx("h1", { className: "text-2xl font-semibold text-gray-900", children: "Tax & CGT" }), _jsxs("p", { className: "text-sm text-gray-500 mt-0.5", children: ["Australian Capital Gains Tax \u2014 FY ", fy - 1, "\u2013", fy] })] }), _jsxs("div", { className: "flex items-center gap-3", children: [_jsxs("div", { className: "relative", children: [_jsx("select", { value: fy, onChange: e => setFy(Number(e.target.value)), className: "appearance-none pl-3 pr-8 py-2 text-sm border border-gray-300 rounded-lg\n                         focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white text-gray-700", children: FY_OPTIONS.map(y => (_jsxs("option", { value: y, children: ["FY ", y - 1, "\u2013", y] }, y))) }), _jsx(ChevronDown, { size: 14, className: "absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none" })] }), _jsxs("button", { onClick: handlePdfDownload, className: "flex items-center gap-1.5 px-3 py-2 text-sm text-white\n                       bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors", children: [_jsx(Download, { size: 14 }), " PDF Report"] }), _jsxs("button", { onClick: handleCsvDownload, className: "flex items-center gap-1.5 px-3 py-2 text-sm text-gray-600\n                       border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors", children: [_jsx(Download, { size: 14 }), " Export CSV"] })] })] }), summaryQ.isLoading ? (_jsxs("div", { className: "flex items-center gap-2 text-gray-400 py-4", children: [_jsx(Loader2, { size: 16, className: "animate-spin" }), " Loading CGT summary\u2026"] })) : summaryQ.data ? (_jsx(CgtSummaryCards, { summary: summaryQ.data })) : null, _jsxs("div", { className: "flex items-start gap-3 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800", children: [_jsx(AlertCircle, { size: 16, className: "flex-shrink-0 mt-0.5" }), _jsx("span", { children: "These calculations are indicative only. Verify all amounts with a registered tax agent before lodging your return. Return of capital adjustments and corporate actions may affect your actual cost base." })] }), _jsx("div", { className: "border-b border-gray-200", children: _jsx("nav", { className: "flex gap-6 -mb-px", children: ['summary', 'events', 'parcels', 'dividends'].map(t => (_jsx("button", { onClick: () => setTab(t), className: clsx('py-3 text-sm font-medium border-b-2 transition-colors', tab === t
                            ...
                            : t === 'events' ? `Disposal Events FY${fy}`
                                : t === 'parcels' ? 'Open Parcels'
                                    : 'Dividends' }, t))) }) }), tab === 'summary' && summaryQ.data && (_jsx(CgtBreakdownTable, { summary: summaryQ.data })), tab === 'events' && (_jsx(CgtEventsTable, { events: eventsQ.data ?? [], isLoading: eventsQ.isLoading, sort: eventSort, sortDir: eventSortDir, onSort: toggleEventSort })), tab === 'parcels' && (_jsx(OpenParcelsTable, { parcels: parcelsQ.data ?? [], isLoading: parcelsQ.isLoading, sort: parcelSort, sortDir: parcelSortDir, onSort: toggleParcelSort })), tab === 'dividends' && (dividendsQ.isLoading ? (_jsxs("div", { className: "flex items-center gap-2 text-gray-400 py-4", children: [_jsx(Loader2, { size: 16, className: "animate-spin" }), " Loading dividends\u2026"] })) : dividendsQ.data ? (_jsx(DividendsView, { data: dividendsQ.data })) : (_jsxs("div", { className: "bg-white rounded-xl border border-gray-200 px-6 py-10 text-center text-gray-400", children: ["No dividend data for FY ", fy - 1, "\u2013", fy] })))] }));
}
// ── CGT Summary Cards ─────────────────────────────────────────────────────────
...
function DividendsView({ data }) {
    return (_jsxs("div", { className: "space-y-6", children: [_jsxs("div", { className: "grid grid-cols-2 lg:grid-cols-4 gap-4", children: [_jsx(TaxCard, { label: "Cash Dividends", value: fmtCcy(data.totalCashDividends), icon: _jsx(TrendingUp, { size: 15 }), positive: true }), _jsx(TaxCard, { label: "Franking Credits", value: fmtCcy(data.totalFrankingCredits), icon: _jsx(Shield, { size: 15 }), positive: true, sub: "30% tax offset" }), _jsx(TaxCard, { label: "Grossed-Up Income", value: fmtCcy(data.totalGrossedUpIncome), icon: _jsx(DollarSign, { size: 15 }), neutral: true, sub: "Taxable income" }), _jsx(TaxCard, { label: "Tax Withheld", value: fmtCcy(data.totalTaxWithheld), icon: _jsx(TrendingDown, { size: 15 }) })] }), _jsxs("div", { className: "bg-white rounded-xl border border-gray-200", children: [_jsxs("div", { className: "px-6 py-3 border-b border-gray-100 flex items-center justify-between", children: [_jsx("span", { className: "text-sm font-semibold text-gray-700", children: "Dividend History" }), _jsxs("span", { className: "text-xs text-gray-400", children: [data.byHolding.length, " holdings"] })] }), _jsx("div", { className: "overflow-x-auto", children: _jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { children: _jsx("tr", { className: "text-xs text-gray-400 border-b border-gray-100", children: ['Ticker', 'Cash Dividend', 'Franking Credits', 'Franking %', 'Grossed-Up', 'Tax Withheld'].map(h => (_jsx("th", { className: "px-4 py-3 text-left font-medium whitespace-nowrap", children: h }, h))) }) }), _jsxs("tbody", { children: [data.byHolding.map((d, i) => (_jsxs("tr", { className: "border-b border-gray-50 last:border-0 hover:bg-gray-50/40", children: [_jsx("td", { className: "px-4 py-3 text-left font-semibold text-gray-900", children: d.ticker }), _jsx("td", { className: "px-4 py-3 text-left tabular-nums text-gray-700", children: fmtCcy(d.cashDividends) }), _jsx("td", { className: "px-4 py-3 text-left tabular-nums text-emerald-600", children: fmtCcy(d.frankingCredits) }), _jsx("td", { className: "px-4 py-3 text-left tabular-nums text-gray-500", children: d.frankingPercentage > 0 ? `${(d.frankingPercentage * 100).toFixed(0)}%` : '—' }), _jsx("td", { className: "px-4 py-3 text-left tabular-nums text-gray-700", children: fmtCcy(d.grossedUpAmount) }), _jsx("td", { className: "px-4 py-3 text-left tabular-nums text-gray-500", children: fmtCcy(0) })] }, i))), data.byHolding.length === 0 && (_jsx("tr", { children: _jsx("td", { colSpan: 6, className: "px-4 py-10 text-center text-gray-400", children: "No dividends recorded for this financial year." }) }))] })] }) })] })] }));
}
