...
import { Plus, Search, Download, Upload, TrendingUp, TrendingDown, ArrowLeftRight, DollarSign, X, Loader2, Pencil, Trash2, CheckSquare, Square, ChevronUp, ChevronDown } from 'lucide-react';
...
function TradeRow({ trade, isSelected, onSelect, onEdit, onDelete }) {
    ...
    return (_jsxs("tr", { className: "border-b border-gray-50 last:border-0 hover:bg-gray-50/50 transition-colors", children: [_jsx("td", { className: "px-5 py-3 w-8", children: _jsx("button", { onClick: onSelect, className: "text-blue-600 hover:text-blue-800", children: isSelected ? _jsx(CheckSquare, { size: 16 }) : _jsx(Square, { size: 16 }) }) }), _jsx("td", { className: "px-5 py-3 text-left text-gray-500 whitespace-nowrap", children: format(parseISO(trade.tradeDate), 'dd MMM yyyy') }), _jsxs("td", { className: "px-5 py-3 text-left", children: [_jsx("span", { className: "font-semibold text-gray-900", children: trade.ticker }), _jsx("span", { className: "text-xs text-gray-400 ml-1", children: trade.exchange })] }), _jsx("td", { className: "px-5 py-3 text-left", children: _jsxs("span", { className: clsx('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium', isBuy && 'bg-emerald-50 text-emerald-700', isSell && 'bg-red-50 text-red-600', isDividend && 'bg-blue-50 text-blue-700', !isBuy && !isSell && !isDividend && 'bg-gray-100 text-gray-600'), children: [isBuy && _jsx(TrendingUp, { size: 10 }), isSell && _jsx(TrendingDown, { size: 10 }), isDividend && _jsx(DollarSign, { size: 10 }), !isBuy && !isSell && !isDividend && _jsx(ArrowLeftRight, { size: 10 }), fmtType(trade.tradeType)] }) }), _jsx("td", { className: "px-5 py-3 text-left tabular-nums text-gray-700", children: trade.quantity.toLocaleString() }), _jsx("td", { className: "px-5 py-3 text-left tabular-nums text-gray-600", children: fmtCcy(trade.price, trade.currency) }), _jsx("td", { className: "px-5 py-3 text-left tabular-nums text-gray-500", children: fmtCcy(trade.fees, trade.currency) }), _jsx("td", { className: "px-5 py-3 text-left tabular-nums font-medium text-gray-900", children: fmtCcy(trade.totalCost, trade.currency) }), _jsx("td", { className: "px-5 py-3 text-left", children: _jsx("span", { className: "text-xs text-gray-400", children: fmtSource(trade.source) }) }), _jsx("td", { className: "px-5 py-3 w-16", children: _jsxs("div", { className: "flex items-center justify-end gap-1", children: [_jsx("button", { onClick: onEdit, className: "p-1 text-gray-400 hover:text-blue-600 rounded hover:bg-blue-50", children: _jsx(Pencil, { size: 14 }) }), _jsx("button", { onClick: onDelete, className: "p-1 text-gray-400 hover:text-red-600 rounded hover:bg-red-50", children: _jsx(Trash2, { size: 14 }) })] }) })] }));
}
// ── Add trade modal ───────────────────────────────────────────────────────────
...
function fmtSource(t) {
    return { MANUAL: 'Manual', BROKER_API: 'Broker API', PDF_IMPORT: 'PDF Import', CSV_IMPORT: 'CSV Import' }[t] ?? t;
}
